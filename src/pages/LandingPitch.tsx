...
const industries = [
  ...
  { name: "Supermercados", icon: ShoppingCart, color: "bg-blue-50 text-blue-600" },
  { name: "Farmácias", icon: PlusIcon, color: "bg-red-50 text-red-600" },
  { name: "Saúde", icon: Stethoscope, color: "bg-green-50 text-green-600" },
  { name: "Serviços", icon: Briefcase, color: "bg-indigo-50 text-indigo-600" },
...
export default function LandingPitch() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 selection:bg-indigo-100 selection:text-indigo-900 font-sans cursor-default overflow-x-hidden">
      {/* Navbar Smart */}
      <motion.nav
        initial={{ y: 0 }}
        animate={{ y: 0 }}
      ...
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between gap-4">
          <div className="flex-shrink-0 flex items-center gap-3">
            <img src={logo} alt="Represente-Me!" className="h-10 md:h-12 w-auto object-contain transition-all" />
          </div>
          ...
          <div className="flex items-center gap-4">
            <Link to="/login" className="px-6 py-3 rounded-2xl bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-xl shadow-slate-200 hover:">
              Entrar
            </Link>
          </div>
        </div>
      ...
      <section className="pt-48 pb-20 px-6">
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            <span className="px-4 py-1.5 rounded-full bg-indigo-50 text-indigo-600 text-xs font-black uppercase tracking-widest border border-indigo-100 mb-8 inline-block shadow-sm">
            ...
            <h1 className="text-5xl md:text-8xl font-black tracking-tight text-slate-900 mb-8 leading-[0.9] text-balance">
              Venda mais com <span className="text-transparent bg-clip-text bg-gradient-to-br from-indigo-600 via-indigo-500 to-indigo-400">Menos Esforço.</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-5">
              <a href="#planos" className="w-full sm:w-auto px-10 py-5 rounded-2xl bg-indigo-600 text-white font-black text-xs uppercase tracking-widest hover:bg-indigo-700 transition-all  hover:-translate-y-1 flex items-center justify-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            {industries.map((item, idx) => (
              <motion.button
                ...
                className="p-8 rounded-[32px] border border-slate-100 bg-white flex flex-col items-center gap-5 hover:shadow-2xl hover:border-indigo-100 transition-all group shadow-sm"
              >
          ...
          <div className="flex flex-col lg:flex-row items-center gap-20">
            ...
            >
              ...
              <h2 className="text-5xl font-black text-slate-900 uppercase tracking-tighter mb-8 leading-[1.1]">
                Controle total da sua <span className="text-indigo-600">Comunicação</span>
              </h2>
            ...
            >
              <div className="absolute inset-0 bg-indigo-600/5 blur-[120px] rounded-full group-hover:bg-indigo-600/10 transition-all pointer-events-none" />
        ...
        <div className="max-w-7xl mx-auto px-6 relative">
          <div className="text-center mb-24">
            ...
            <div className="flex items-center justify-center gap-4">
              <span className={`text-[10px] font-black uppercase tracking-widest transition-colors ${billingCycle === "monthly" ? "text-indigo-600" : "text-slate-400"}`}>Plano Mensal</span>
              <button 
                type="button"
              ...
              <span className={`text-[10px] font-black uppercase tracking-widest transition-colors ${billingCycle === "annual" ? "text-indigo-600" : "text-slate-400"}`}>Plano Anual</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch">
            {plans.map((plan, idx) => (
              ...
              >
                {plan.popular && (
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-indigo-500 text-white text-[9px] font-black uppercase tracking-[0.2em] px-6 py-2 rounded-full shadow-lg shadow-indigo-500/30">
                    Recomendado
                  </div>
                )}
                ...
                <div className="space-y-5 mb-12 flex-1">
                  {plan.features.map((feat, fidx) => (
                    <div key={fidx} className="flex items-start gap-4">
                      <div className={`p-1.5 rounded-lg flex-shrink-0 mt-0.5 ${plan.popular ? "bg-white/10" : "bg-indigo-50"}`}>
                        <Check className={`w-3.5 h-3.5 ${plan.popular ? "text-indigo-400" : "text-indigo-600"}`} />
                      </div>
        ...
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={fidx} className="p-8 rounded-3xl bg-white border border-slate-100 shadow-sm hover:border-indigo-100 transition-all">
      ...
      <section className="py-32 px-6">
        <div className="max-w-5xl mx-auto rounded-[64px] bg-slate-900 p-12 md:p-24 text-center relative overflow-hidden group">
          <div className="absolute inset-0 bg-indigo-600/10 blur-[100px] group-hover:bg-indigo-600/20 transition-all" />
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} className="relative">
            <h2 className="text-4xl md:text-6xl font-black text-white uppercase tracking-tighter mb-8 leading-none">
              Pronto para ser um <br /> <span className="text-indigo-400 text-5xl md:text-8xl">SUPER REPRESENTANTE?</span>
            </h2>
            <p className="text-slate-400 font-medium text-lg max-w-xl mx-auto mb-12">
              Junte-se a mais de 2.000 que já alavancaram mais de 150% de suas vendas com a Represente-Me!
            </p>
            <a href="#planos" className="px-12 py-6 rounded-2xl bg-indigo-500 text-white font-black text-xs uppercase tracking-widest hover:bg-indigo-400 transition-all shadow-2xl shadow-indigo-500/20 inline-block">
              Começar agora mesmo
            </a>
          </motion.div>
        </div>
      ...
      <footer className="py-12 border-t border-slate-100 bg-white">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:row items-center justify-between gap-8">
          <img src={logo} alt="Represente-Me!" className="h-8 md:h-10 w-auto opacity-50" />
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center">
            © 2026 Represente-Me! — Tecnologia para Representações Comerciais
          </p>
