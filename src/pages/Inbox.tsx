      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-zinc-100 flex items-center gap-3 uppercase tracking-tight">
            <Mail className="w-8 h-8 text-emerald-600" /> Mensagens
          </h1>
      ...
      <div className="flex-1 flex gap-6 min-h-0">
        ...
        <div className="w-96 bg-white dark:bg-zinc-900 rounded-[40px] border border-slate-200 dark:border-zinc-800 flex flex-col overflow-hidden shadow-sm">
          <div className="p-6 border-b dark:border-zinc-850">
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-3 bg-slate-50 dark:bg-zinc-950 border-none rounded-2xl text-xs font-bold transition-all focus:ring-2 focus:ring-emerald-500"
              />
            </div>
          </div>
          <div className="flex-1 overflow-y-auto p-1 space-y-1">
            {mockMessages.map((msg) => (
              ...
              >
                {selected.id === msg.id && (
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 w-1.5 h-12 bg-emerald-600 rounded-full" />
                )}
                <div className={`flex justify-between items-start ${selected.id === msg.id ? "pl-4" : ""}`}>
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-2xl bg-emerald-100 dark:bg-emerald-900/40 flex items-center justify-center text-emerald-600 font-black text-sm uppercase">
                      {msg.avatar}
                    </div>
                    <div>
                      <h4 className={`text-xs font-black uppercase tracking-tight ${!msg.read && selected.id !== msg.id ? "text-emerald-600" : "text-slate-900 dark:text-zinc-100"}`}>
                        {msg.sender}
                      </h4>
                      <p className="text-[10px] font-bold text-slate-400 uppercase">{msg.time}</p>
                    </div>
                  </div>
                  {!msg.read && <div className="w-2 h-2 bg-emerald-600 rounded-full mt-1" />}
                </div>
        ...
        <div className="flex-1 bg-white dark:bg-zinc-900 rounded-[40px] border border-slate-200 dark:border-zinc-800 flex flex-col overflow-hidden shadow-sm relative">
          <div className="p-6 border-b dark:border-zinc-850 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <span className="px-2 py-0.5 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 text-[9px] font-black uppercase rounded-lg border border-emerald-100 dark:border-emerald-900/40">
                  {selected.tag}
                </span>
              </div>
            </div>
          ...
          <div className="p-6 border-t dark:border-zinc-850">
            <div className="bg-slate-50 dark:bg-zinc-950 rounded-[32px] p-2 flex flex-col gap-2 border border-slate-100 dark:border-zinc-800 transition-all focus-within:ring-2 focus-within:ring-emerald-500">
              <textarea 
              ...
              <div className="flex items-center justify-between p-2">
                ...
                <button className="px-8 py-3 bg-emerald-600 text-white rounded-3xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-emerald-500/20 hover:bg-emerald-700 active:scale-95 transition-all flex items-center gap-2">
