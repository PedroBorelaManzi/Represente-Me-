...
const plans = {
  exclusivo: { name: 'Exclusivo', price: 97, bump: 'profissional', bumpPrice: 50 },
  profissional: { name: 'Profissional', price: 147, bump: 'corporativo', bumpPrice: 50 },
  corporativo: { name: 'Corporativo', price: 197, bump: null }
};
...
export default function Checkout() {
  ...
  const totalPrice = isBumpChecked && selectedPlan.bump 
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-zinc-950 font-sans text-slate-900 dark:text-zinc-100 selection:bg-emerald-500/30">
                ...
                {step === 1 ? (
                  ...
                  >
                      ...
                      <div className="grid grid-cols-3 gap-3 p-1.5 bg-slate-50 dark:bg-zinc-950 rounded-3xl border border-slate-100 dark:border-zinc-800">
                        <button 
                          ...
                          className={cn(
                            ...
                            paymentMethod === 'card' ? "bg-white dark:bg-zinc-900 shadow-sm text-emerald-600" : "text-slate-400 hover:text-slate-600"
                          )}
                        >
                      ...
                      <div className="space-y-6 pt-4">
                        ...
                        {paymentMethod === 'pix' && (
                          <div className="text-center p-8 bg-emerald-50 dark:bg-emerald-500/5 rounded-3xl border border-emerald-100 dark:border-emerald-500/20">
                            <div className="w-16 h-16 bg-emerald-500 text-white rounded-2xl flex items-center justify-center mx-auto mb-4">
                            ...
                            <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest leading-relaxed">O QR Code será gerado após clicar no botão de finalizar.</p>
                          </div>
                        )}
                        ...
                        {paymentMethod === 'boleto' && (
                          <div className="text-center p-8 bg-slate-50 dark:bg-zinc-950 rounded-3xl border border-slate-100 dark:border-zinc-800">
                            ...
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed">A liberação ocorre em até 2 dias úteis após o pagamento.</p>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <button 
                      type="submit"
                      disabled={loading}
                      className="w-full bg-emerald-600 text-white py-5 rounded-[24px] font-black uppercase text-xs tracking-[0.2em] hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-500/20 flex items-center justify-center gap-4 active:scale-95"
                    >
                      {loading ? (
            ...
            <div className="sticky top-28 space-y-6">
              ...
              <div className="bg-white dark:bg-zinc-900 rounded-[40px] p-8 border border-slate-100 dark:border-zinc-800 shadow-2xl space-y-8">
                <div className="flex items-center justify-between">
                  ...
                  <div className="w-10 h-10 bg-slate-50 dark:bg-zinc-800 rounded-xl flex items-center justify-center">
                ...
                <div className="space-y-4">
                  <div className="flex items-start justify-between">
                    ...
                    <p className="text-sm font-black text-slate-900 dark:text-zinc-100">R$ {selectedPlan.price.toFixed(2).replace('.', ',')}</p>
                  </div>
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="space-y-1">
                          ...
                          <h4 className={cn("text-[11px] font-black uppercase tracking-tight", isBumpChecked ? "text-white" : "text-slate-900 dark:text-white")}>
                            Mudar para o Plano {plans[selectedPlan.bump as keyof typeof plans].name}
                          </h4>
                          <p className={cn("text-[9px] font-medium leading-relaxed", isBumpChecked ? "text-white/80" : "text-slate-500")}>
                            Leve automação com IA e relatórios avançados por apenas <strong className={isBumpChecked ? "text-white" : "text-emerald-600"}>+ R$ {selectedPlan.bumpPrice},00/mês</strong>.
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="pt-4 mt-4 border-t border-slate-100 dark:border-zinc-800/50 space-y-4">
                     <div className="flex items-center justify-between">
                        ...
                        <p className="text-xs font-bold text-slate-900 dark:text-zinc-100">R$ {totalPrice.toFixed(2).replace('.', ',')}</p>
                     </div>
              ...
              {/* Prova Social / Benefícios Rápidos */}
              <div className="bg-emerald-600 rounded-[32px] p-8 text-white shadow-xl shadow-emerald-600/20 space-y-6">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-tight">Garantia de 7 dias</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <CheckCircle2 className="w-4 h-4 text-emerald-300" />
                    <span className="text-[10px] font-bold uppercase tracking-tight">Suporte Prioritário</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <CheckCircle2 className="w-4 h-4 text-emerald-300" />
                    <span className="text-[10px] font-bold uppercase tracking-tight">Sem fidelidade</span>
                  </div>
                </div>
              </div>
      ...
      <footer className="max-w-6xl mx-auto px-6 py-12 text-center border-t border-slate-100 dark:border-zinc-800/50 mt-12">
        ...
        <div className="flex items-center justify-center gap-4 opacity-50">
          <img src="https://www.asaas.com/assets/img/brand/asaas-logo-blue.svg" alt="Asaas" className="h-4 dark:invert" />
        </div>
      </footer>
    </div>
  );
