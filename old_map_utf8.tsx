...
export default function MapPage() {
  ...
  const [center, setCenter] = useState<[number, number]>([-15.793889, -47.882778]); // Bras├¡lia - Centro do Brasil
  ...
  useEffect(() => {
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          console.error("Erro ao obter localiza├º├úo:", error);
        }
      );
    }
  }, []);
  ...
  const handleMarkerDrag = async (id: string, latlng: { lat: number, lng: number }) => {
    ...
    if (!error) {
      ...
      toast.success("Localiza├º├úo atualizada!");
    } else {
      toast.error("Erro ao salvar localiza├º├úo");
    }
  };

  const handleDeleteClient = async (id: string, name: string) => {
    if (!window.confirm(`Deseja realmente excluir o cliente "${name}"? Esta a├º├úo n├úo pode ser desfeita.`)) return;
  ...
  const handleCnpjLookup = async () => {
    ...
    if (!cleanedCnpj || cleanedCnpj.length !== 14) {
      toast.error("Insira um CNPJ v├ílido.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) throw new Error("CNPJ n├úo encontrado");
      ...
      toast.error("CNPJ n├úo encontrado. Preencha manualmente.");
    } finally {
    ...
    <div className="h-full flex flex-col gap-8 md:gap-10 pb-4">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-zinc-400 mt-2 font-medium">Visualiza├º├úo geo-estrat├®gica da sua carteira de clientes.</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
          <form onSubmit={handleMapSearch} className="relative w-full sm:w-96 group">
            ...
            <input
              ...
              placeholder="Buscar Cliente ou Endere├ºo..."
            />
          </form>
          <button 
                ...
                <div className="p-4 min-w-[280px] bg-white dark:bg-zinc-900">
                  ...
                  <div className="space-y-3 mb-6">
                    <div className="flex items-center gap-3 text-xs font-bold text-slate-600 dark:text-zinc-400 bg-slate-50 dark:bg-zinc-800/50 p-3 rounded-2xl">
                      ...
                      <span className="truncate">{company.address || "Endere├ºo n├úo informado"}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 justify-center py-2 bg-amber-50 dark:bg-amber-900/20 rounded-xl">
                    ...
                    <span className="text-[9px] font-black uppercase tracking-tighter text-amber-700 dark:text-amber-500">Arraste para ajustar posi├º├úo exata</span>
                  </div>
                </div>
              </Popup>
            </Marker>
            ...
            >
               <div className="p-12 border-b dark:border-zinc-850 flex items-center justify-between bg-slate-50/50 dark:bg-zinc-950/20">
                <div>
                  ...
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Sincroniza├º├úo com o Ecossistema Territorial</p>
                </div>
              ...
              <form onSubmit={handleCreateLocation} className="p-12 space-y-10">
                ...
                <div className="space-y-4">
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">Identifica├º├úo do Ponto</label>
                  <input required type="text" value={newLocation.name} onChange={e => setNewLocation({...newLocation, name: e.target.value})} className="w-full px-8 py-5 bg-slate-50 dark:bg-zinc-950 border border-slate-100 dark:border-zinc-800 rounded-[32px] text-sm font-black text-slate-900 dark:text-zinc-100 outline-none focus:ring-8 focus:ring-emerald-500/10 transition-all" placeholder="Raz├úo Social ou Nome Fantasia" />
                </div>

                <div className="space-y-4">
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">Localiza├º├úo Territorial</label>
                  <textarea 
