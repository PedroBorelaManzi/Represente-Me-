        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <a href="#planos" className="text-sm font-medium text-slate-600 hover:text-indigo-600 transition-colors">Planos</a>
              <Link to="/login" className="text-sm font-medium text-slate-600 hover:text-indigo-600 transition-colors">Login</Link>
              <Link to="/login?mode=signup" className="text-sm font-medium bg-indigo-600 text-white px-4 py-2 rounded-full hover:bg-indigo-700 transition-colors shadow-sm shadow-indigo-200">
                Começar Agora
              </Link>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link to="/login?mode=signup" className="inline-flex items-center justify-center px-8 py-4 text-base font-medium rounded-full text-white bg-indigo-600 hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200 hover:shadow-xl hover:-translate-y-0.5">
      ...
      <section id="funcionalidades" className="py-24 bg-white border-y border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-bold text-slate-900 sm:text-4xl">Funcionalidades Poderosas</h2>
...
function PricingCard({ title, description, price, features, buttonText, popular }: { title: string, description: string, price: string, features: string[], buttonText: string, popular?: boolean }) {
  return (
    <div className={`bg-white rounded-3xl p-8 border ${popular ? 'border-indigo-500 shadow-xl shadow-indigo-100 relative' : 'border-slate-200 shadow-sm'}`}>
      ...
      <Link to="/login?mode=signup" className={`block w-full py-3 px-4 rounded-xl text-center font-semibold transition-colors ${popular ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-slate-100 text-slate-900 hover:bg-slate-200'}`}>
        {buttonText}
      </Link>
    </div>
  );
